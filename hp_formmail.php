<?php
$FM_VERS = "8.11";      // script version

/* ex:set ts=4 sw=4 et:
 * FormMail PHP script from Tectite.com.  This script requires PHP 4 or later.
 * Copyright (c) 2001-2008 Root Software and Open Concepts (Vic) Pty Ltd
 ...
 *      line 239 in this file - replace "yourhost\.com" with your mail server's
...
$TARGET_EMAIL = array(EMAIL_NAME."@<!-- Couldn't get value of variable domainname: Model is not a TemplateScalarModel, it's a class psoft.util.freemarker.TemplateMethodWrapper
. -->
$");

            /* Help: http://www.tectite.com/fmdoc/def_alert.php */
define("DEF_ALERT","wiltsecreations@gmail.com");
...
$FORM_INI_FILE = "/hsphere/local/home/wiltsecc/artbyclinton.com/hp_formmail_noimg/formmail.ini";
...
$SCRATCH_PAD = "/hsphere/local/home/wiltsecc/artbyclinton.com/hp_formmail_noimg/tmp";
...
function LoadBuiltinLanguage()
{
    ...
    $aMessages[MSG_ATTACK_PAGE] = 'Your form submission has been rejected '.
                                  ...
                                  ' target="_blank">more information about this error</a>.';
...
function ZapSession()
{
    global  $aSessionVarNames,$aSessionVars;
...
$SPECIAL_FIELDS = array(
        ...
        "replyto",      // comma-separated list of email addresses to who replies should be sent
    ...
    function    CheckAddress($s_email)
    {
        ...
        else
        {
            for ($ii = 0 ; $ii < count($this->_aTargetPatterns) ; $ii++)
                if (eregi($this->_aTargetPatterns[$ii],$s_email))
                {
                    $b_is_valid = true;
                    break;
                }
...
function IsFieldSet($s_fld,$a_main_vars)
{
    ...
    return (isset($a_main_vars[$s_fld]) ||
            (FILEUPLOADS &&
                (isset($aFileVars[$s_fld]) ||
                    (isset($aSessionVars["FormSavedFiles"]) &&
                        isset($aSessionVars["FormSavedFiles"][$s_fld]))
                )
            ));
}
...
function DeleteFileInfo($s_fld)
{
    ...
    if (FILEUPLOADS)
    {
        if (isset($aSessionVars["FormSavedFiles"]) &&
                isset($aSessionVars["FormSavedFiles"][$s_fld]))
            unset($aSessionVars["FormSavedFiles"][$s_fld]);
...
function GetFileInfo($s_fld)
{
    ...
    {
            //
            // Look in FormSavedFiles first, so that submitting an
            // empty file field doesn't take precedence.
            //
        if (isset($aSessionVars["FormSavedFiles"]) &&
                isset($aSessionVars["FormSavedFiles"][$s_fld]))
            $a_upload = $aSessionVars["FormSavedFiles"][$s_fld];
...
function UnMangle($email)
{
    ...
    //
function CheckEmailAddress($s_addr,&$s_valid,&$s_invalid,$b_check = true)
{
    global  $ValidEmails;
...
function    GetSavedFileNames($a_values)
{
    ...
    if (isset($aSessionVars["FormSavedFiles"]))
    {
        foreach ($aSessionVars["FormSavedFiles"] as $s_fld=>$a_upload)
...
function MultiFormLogic()
{
    ...
    if ($SPECIAL_VALUES["multi_start"] == 1)
    {
        ...
        $iFormIndex = $aSessionVars["FormIndex"] = 0;   // zero is the first form, which was
            ...
            //
        unset($aSessionVars["FormSavedFiles"]);
...
function DetectAttacks($a_fields)
{
    ...
    if (!$b_attacked && !empty($ATTACK_DETECTION_REVERSE_CAPTCHA))
            if (DetectRevCaptchaAttack($ATTACK_DETECTION_REVERSE_CAPTCHA,$a_fields,$s_attack,$s_info))
                $b_attacked = true;
    ...
    if ($b_attacked)
    {
        ...
        else
        {
            ...
            CreatePage(GetMessage(MSG_ATTACK_PAGE,array("SERVER"=>$SERVER)));
        }
        exit;
    }
}
    ...
    //
function    DetectRevCaptchaAttack($a_revcap_spec,$a_form_data,&$s_attack,&$s_info)
{
...
class   SessionAccess
{
    ...
    function    CopyIn(&$a_vars,$b_overwrite_empty)
    {
        ...
        foreach ($this->_aAccessList as $s_var_name)
            if (isset($aSessionVars[$s_var_name]))
                if (!isset($a_vars[$s_var_name]) ||
                        ($b_overwrite_empty &&
                        IsFieldEmpty($a_vars[$s_var_name])))
                {
                    $a_vars[$s_var_name] = $aSessionVars[$s_var_name];
                    $n_copied++;
                };
        return ($n_copied);
    }
    ...
    function    CopyOut(&$a_vars,$a_fields = array())
    {
        ...
        foreach ($this->_aAccessList as $s_var_name)
            if (isset($a_vars[$s_var_name]))
                ...
                {
                    $aSessionVars[$s_var_name] = $a_vars[$s_var_name];
                    $n_copied++;
                };
        return ($n_copied);
    }
...
if (isset($SPECIAL_VALUES["imgverify"]) && $SPECIAL_VALUES["imgverify"] !== "")
{
    ...
    if (isset($aSessionVars["VerifyImgString"]))
    {
        if (strtoupper(str_replace(" ","",$SPECIAL_VALUES["imgverify"])) !==
                    strtoupper($aSessionVars["VerifyImgString"]))
...
if (!empty($SPECIAL_VALUES["fmcompute"]))
{
    ...
    RegisterData($aAllRawValues,MergeFileArrays($aFileVars,$aSessionVars["FormSavedFiles"]));
        //
...
else
{
    ...
    //
if (isset($SPECIAL_VALUES["arverify"]) && $SPECIAL_VALUES["arverify"] !== "")
{
